...
module.exports = {
  ...
  CompletedOrders: async (req, res, next) => {
    try {
      ...
      const ordersWithTotalPrice = completedOrders.map((order) => {
        ...
        // Tính tổng thu nhập của cả đơn hàng
  ...
  listorders1: async (req, res, next) => {
    try {
      const conditionFind = {};
      ...
      const completedOrders = await Order.find(conditionFind)
        ...
        .populate({
          path: "tableId",
          model: "Table",
          select: "name", // Chọn các trường cần lấy từ bảng Table
        })
        .lean();
      ...
      const ordersWithTotalPrice = completedOrders.map((order) => {
        // Tính tổng tiền của tất cả các orderDetail trong đơn hàng
        let totalOrderPrice = 0;
        let totalProductQuantity = 0; // Tổng số sản phẩm được mua
        const orderDetailsWithTotalPrice = order.orderDetails.map(
          (orderDetails) => {
            const totalOrderDetailPrice =
              orderDetails.price * orderDetails.quantity;

            // Tính tổng số sản phẩm được mua
            totalProductQuantity += 1; // Thêm 1 cho mỗi sản phẩm trong chi tiết đơn hàng
        ...
        const orderData = {
          order: {
            _id: order._id,
            createdDate: order.createdDate, // Thêm thông tin về ngày tạo đơn hàng
            paymentType: order.paymentType,
            status: order.status,
            discount: order.discount,
            shippingAddress: order.shippingAddress,
